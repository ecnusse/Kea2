...
- [WeChat's iExplorer]() --- 微信内部测试平台（配备交互式UI工具，便于编写脚本）

- [WeChat Payment's UAT]() --- 微信支付内部测试平台（通过从系统规格合成性质，实现全自动基于性质的测试）
...
```
> 如果你使用清华或中科大镜像，可能升级失败，因为这些镜像可能未同步最新版本。此时，你可以手动指定最新版本安装，或者直接使用 `pypi.org`，例如：  
...
> 快速测试会自动下载 `omninotes.apk`。若下载失败，请从Kea2仓库（顶层目录）复制 `omninotes.apk` 到工作目录，再次执行快速测试命令。
...
## 特性 4（实验性功能，脚本与遍历的混合测试）

> 此功能仍在开发中，期待你的反馈！有兴趣请联系我们。

Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*已有的UI测试脚本通常覆盖重要的App功能且能到达深层状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此功能允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot探索App。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态。（Fastbot自身难以完成登录）

### 示例

完整示例见 [guide_scripts.py](guide_scripts.py)。

通过装饰器`@interruptable`，你可以标记测试用例为“可中断”，使Kea2识别该脚本并在返回后启动模糊测试。
...
Kea2会在每次测试结束后自动生成HTML测试报告。你可以在工作目录下的`output/`中找到报告。

你也可以通过命令 `kea2 report` 手动生成测试报告（详情见 `kea2 report -h`）。

此外，可以通过 `kea2 merge` 合并多次测试的报告（详情见 `kea2 merge -h`）。合并报告在多次测试时非常有用。
...
- [Kea2 101（Kea2从0到1入门教程与最佳实践，建议新手阅读）](https://sy8pzmhmun.feishu.cn/wiki/EwaWwPCitiUJoBkIgALcHtglnDK?from=from_copylink)  
