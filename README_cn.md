...
请联系Xixian Liang，邮箱 [xixian@stu.ecnu.edu.cn](xixian@stu.ecnu.edu.cn)，并提供你的微信ID或二维码，以便邀请加入微信群讨论组。当然，我们也随时在GitHub上回答你的问题和反馈。
...
Kea2目前基于[Fastbot](https://github.com/ecnusse/Fastbot_Android) 3.0（原始[FastBot](https://github.com/bytedance/Fastbot_Android) 2.0的修改和增强版本），*一款字节跳动工业级自动化UI测试工具*，以及[uiautomator2](https://github.com/openatx/uiautomator2)，*一款易用且稳定的Android自动化库*构建。  
...
- [OPay Business](https://play.google.com/store/apps/details?id=team.opay.pay.merchant.service) — 一款金融与支付应用。OPay使用Kea2在POS机和移动设备上进行回归测试。

- [微信iExplorer]() — 微信内部测试平台（配备交互式UI工具以简化脚本编写）

- [微信支付UAT]() — 微信支付内部测试平台（通过从系统规格综合性质，实现全自动基于性质的测试）
...
```
> 如果你使用清华或中科大等镜像站点，可能会升级失败，因为这些站点可能尚未同步最新版本。此时你可以尝试手动指定最新版本安装，或者直接使用 `pip install kea2-python -i https://pypi.org/simple`。
...
> 如是首次运行Kea2，此步骤是必需的。若升级Kea2，也建议重新执行此步骤以确保新配置生效。
...
> 快速测试会自动下载 `omninotes.apk`。如果下载失败，请从Kea2仓库（顶层目录）复制 `omninotes.apk` 到你的工作目录，再次执行快速测试命令。
...
Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*现有的UI测试脚本通常覆盖重要的App功能，能够达到深层的App状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot探索App。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态。（Fastbot自身难以轻松完成登录）

### 示例

`hybridetest_examples`中有四个示例脚本，分别对应不同形式的用户脚本，展示如何在已有代码中启动kea2。
...
1. 你可以通过修改`if`条件来控制是否执行你写的kea2相关代码，方便在同一脚本中启用或禁用kea2操作。这里以环境变量为例。
2. 由于kea2由u2驱动，如果appium脚本想启动kea2，需要先关闭appium会话。记得在`desired_caps`中配置 `"noReset": True`，避免关闭会话时重置应用。
3. 你需要将以下代码模板插入到已有测试用例中：你可以在注释部分添加自己的hook逻辑，包括启动或停止appium会话、清理实例等，具体取决于你的setup和teardown设计。除此之外，只需配置`option`参数和`configs_path`参数（即`configs`目录所在路径，通常是执行`kea2 init`的目录），然后传入`run_kea2_testing`函数。
...
if os.environ.get('KEA2_HYBRID_MODE', '').lower() == 'true': 
    ...
    result = self.tester.run_kea2_testing(
        ...
        configs_path = None  # 默认，如果你的configs文件夹位于根目录，则可省略此参数。           
    )
    
    # 重启driver会话或清理实例
    # ...
    
    return  # 这会使本测试用例后续步骤不执行
```

## 测试报告

Kea2会在每次测试结束后自动生成HTML格式的测试报告。你可以在工作目录下的 `output/` 文件夹中找到报告。

你也可以通过命令 `kea2 report` 手动生成测试报告（详情见 `kea2 report -h`）。

此外，可以通过命令 `kea2 merge` 合并多次测试会话的测试报告（详情见 `kea2 merge -h`）。合并报告对于多次测试非常有用。

你可以查看Opay提供的[测试报告示例](https://ecnusse.github.io/Kea2_sample_report/)(感谢！)。更多测试报告细节请参见[文档](docs/test_report_introduction.md)。

## 文档（更多文档）

### :blue_book: [用户手册](docs/manual_en.md)（重要！）

你可以查阅[用户手册](docs/manual_en.md)，内容包括：
- Kea2在微信上的使用示例（中文）；
