...
请联系Xixian Liang，邮箱 [xixian@stu.ecnu.edu.cn](xixian@stu.ecnu.edu.cn)，并提供你的微信ID或二维码以便邀请加入微信群。当然，我们也会在GitHub上回答你的问题和反馈。
...
> 快速测试会自动下载 `omninotes.apk`。若下载失败，请手动从Kea2仓库（顶层目录）复制 `omninotes.apk` 到你的工作目录，再次执行快速测试命令。
...
> 此功能仍在开发中，期待你的反馈！如感兴趣请联系我们。

Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*已有的UI测试脚本通常覆盖重要功能且能达到深层状态，因此可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此功能允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot进行探索。这样你可以先完成登录，然后启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态（Fastbot自身难以完成登录）。

### 示例

完整示例见[guide_scripts.py](guide_scripts.py)。

通过装饰器`@interruptable`标记测试用例为“可中断”，使Kea2识别该脚本并在其返回后启动模糊测试。

由于模糊测试后App状态可能不可预测，Kea2提供`common_teardown`函数用于清理前后脚本间环境。该函数可在`configs/teardown.py`中手动指定。
...
Kea2会在每次测试结束后自动生成HTML格式的测试报告，报告位于你工作目录下的`output/`目录。

你也可以通过命令 `kea2 report` 手动生成测试报告（详情见 `kea2 report -h`）。

此外，可以通过 `kea2 merge` 合并多次测试的报告（详情见 `kea2 merge -h`）。合并报告在多次测试时非常有用。

你可以查看Opay提供的[测试报告示例](https://ecnusse.github.io/Kea2_sample_report/)，更多测试报告细节见[文档](docs/test_report_introduction.md)。
