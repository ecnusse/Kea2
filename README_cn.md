...
请联系Xixian Liang，邮箱 [xixian@stu.ecnu.edu.cn](xixian@stu.ecnu.edu.cn)，并附上你的微信ID或二维码，以便邀请加入微信群。当然，我们也随时在GitHub上回答你的问题和反馈。

#### Github仓库链接 [https://github.com/ecnusse/Kea2](https://github.com/ecnusse/Kea2)  
...
Kea2目前基于[Fastbot](https://github.com/ecnusse/Fastbot_Android) 3.0（原始[FastBot](https://github.com/bytedance/Fastbot_Android) 2.0的修改/增强版本，*字节跳动工业级自动化UI测试工具*）和[uiautomator2](https://github.com/openatx/uiautomator2)（*易用且稳定的Android自动化库*）构建。  
Kea2当前支持[Android](https://en.wikipedia.org/wiki/Android_(operating_system))应用。
...
- **特性 2**（自定义测试场景\事件序列\黑白名单\黑白控件[^1]）：运行Fastbot时可自定义测试场景（例如测试特定App功能、执行特定事件序列、进入指定UI页面、达到特定App状态、将指定Activity/UI控件/UI区域加入黑名单），并且在*python*脚本语言和[uiautomator2](https://github.com/openatx/uiautomator2)驱动的支持下，这些自定义功能具有高度可用性和灵活性；
...
- [OPay Business](https://play.google.com/store/apps/details?id=team.opay.pay.merchant.service) — 一款金融支付应用（每日活跃用户达2000万）。OPay使用Kea2对POS机和移动设备进行回归测试。
...
如果你之前已安装过Kea2，可以升级到最新版本：

```bash
python3 -m pip install -U kea2-python
```

> 如果你使用清华、USTC等镜像站点，升级可能失败，因为这些站点可能尚未同步最新版本。此时，你可以尝试手动指定最新版本安装，或者直接使用 `pypi.org`，例如：  
...
> 如果是首次运行Kea2，此初始化步骤是必需的。升级后也建议重新执行此步骤，以确保新配置生效。
...
Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*已有的UI测试脚本通常覆盖重要的App功能且能到达深层状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用已有脚本，在脚本中设置断点（即可中断点），并启动Fastbot在每个断点后探索App。通过此功能，你可以先完成登录，然后启动Fastbot探索登录后的App状态（Fastbot自身难以完成登录操作）。
...
- [u2_unittest_example.py](hybridetest_examples\u2_unittest_example.py)：基于unittest组织的u2脚本。
- [u2_pytest_example.py](hybridetest_examples\u2_pytest_example.py)：基于pytest组织的u2脚本。
- [appium_unittest_example.py](hybridetest_examples\appium_unittest_example.py)：基于unittest组织的appium脚本。
- [appium_pytest_example.py](hybridetest_examples\appium_pytest_example.py)：基于pytest组织的appium脚本。
...
3. 你需要在已有测试用例中插入如下代码模板：你可以在注释部分添加自己的hook逻辑，包括启动或关闭appium会话、清理实例等，具体取决于你的setup和teardown设计。除此之外，只需配置`option`参数和`configs_path`参数（即`configs`目录所在路径，通常是执行`kea2 init`的目录），然后传入`run_kea2_testing`函数。
...
if os.environ.get('KEA2_HYBRID_MODE', '').lower() == 'true': 
    ...
    result = self.tester.run_kea2_testing(
        ...
        configs_path = None  # 默认，如果你的configs文件夹在根目录，则可省略。           
    )
    
    # 重启driver会话或清理实例
    # ...
    
    return  # 使该测试用例后续步骤不执行
```
...
使用`-s/--sync`参数可在生成报告前从设备同步数据，使用`-p/--path`指定一个或多个结果目录。例如：
...
> 你可以查看Opay提供的示例[test report](https://ecnusse.github.io/Kea2_sample_report/)，更多测试报告细节请参见[文档](docs/test_report_introduction.md)。
