...
请联系Xixian Liang，邮箱 [xixian@stu.ecnu.edu.cn](xixian@stu.ecnu.edu.cn)，并附上你的微信ID或二维码，以便邀请你加入微信群。当然，我们也随时在GitHub上回答你的问题和反馈。
...
Kea2是一个易用的移动应用模糊测试工具。其核心*创新点*是能够将自动化UI测试与脚本（通常由人工编写）融合，从而赋能自动化UI测试以人类智能，有效发现*崩溃错误*及*非崩溃功能（逻辑）错误*。
...
> 快速测试会自动下载`omninotes.apk`。如果下载失败，请将Kea2仓库根目录下的`omninotes.apk`复制到当前工作目录，再次执行快速测试命令。
...
> 此特性仍在开发中，期待你的反馈！如有兴趣，请联系我们。

Kea2支持复用已有的UI测试脚本。我们受到以下理念启发：*已有的UI测试脚本通常覆盖重要的App功能且能到达深层状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用该脚本，在脚本中设置若干断点（即可中断点），并在每个断点后启动Fastbot进行探索。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态（Fastbot自身难以完成登录操作）。

### 示例

完整示例见[guide_scripts.py](guide_scripts.py)。

通过装饰器`@interruptable`，你可以标记测试用例为“可中断”，Kea2据此识别脚本并在返回后启动模糊测试。

由于模糊测试后App状态可能不可预测，Kea2提供了`common_teardown`函数，用于清理前后脚本间环境。该函数可在`configs/teardown.py`中手动指定。

完整示例见`guide_scripts.py`、`property_omninotes.py`和`configs/teardown.py`。你可以运行以下命令之一：
...
你也可以通过命令`kea2 report`手动生成测试报告（详见`kea2 report -h`）。

此外，`kea2 merge`命令支持合并多次测试的报告（详见`kea2 merge -h`）。合并报告对于多次测试非常有用。
