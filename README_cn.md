...
Kea2目前基于[Fastbot](https://github.com/ecnusse/Fastbot_Android) 3.0（原始[FastBot](https://github.com/bytedance/Fastbot_Android) 2.0的修改/增强版本），*一款来自字节跳动的工业级自动化UI测试工具*，以及[uiautomator2](https://github.com/openatx/uiautomator2)，*一款易用且稳定的Android自动化库*构建。  
...
- [OPay Business](https://play.google.com/store/apps/details?id=team.opay.pay.merchant.service) — 一款金融与支付应用（每日活跃用户达2000万）。OPay使用Kea2进行POS机及移动设备的回归测试。

- [微信iExplorer]() — 微信内部测试平台（配备交互式UI工具，简化脚本编写）
...
```
> 如果你使用清华、USTC等镜像站点，可能升级失败，因为这些站点可能尚未同步最新版本。此时可以尝试手动指定版本安装，或直接使用官方PyPI源：`pip install kea2-python -i https://pypi.org/simple`。
...
> 如果是首次运行Kea2，此初始化步骤必需。升级后建议重新执行此步骤以确保新配置生效。
...
> 快速测试会自动下载`omninotes.apk`。若下载失败，请从Kea2仓库根目录复制`omninotes.apk`到你的工作目录，再次执行快速测试命令。
...
Kea2支持复用已有的UI测试脚本。我们的灵感来源于：*已有的UI测试脚本通常覆盖重要的App功能并能达到深层App状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot探索App。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层状态。（Fastbot自身难以轻松完成登录操作）
...
2. 由于kea2由u2驱动，如果appium脚本想启动kea2，需先关闭appium会话。记得在`desired_caps`中配置`"noReset": True`，避免关闭会话时重置应用。

3. 你需要将以下代码模板插入已有测试用例中：你可以在注释部分添加自己的hook逻辑，包括启动或停止appium会话、清理实例等，具体取决于你的setup和teardown设计。除此之外，只需配置`option`参数和`configs_path`参数（即`configs`目录所在路径，通常是执行`kea2 init`的目录），然后传入`run_kea2_testing`函数。
...
if os.environ.get('KEA2_HYBRID_MODE', '').lower() == 'true': 
    '''
    注意：此处的if条件可根据项目实际情况修改，环境变量形式仅为示例。    
    '''
...
- [Kea2与基于性质测试的常见问题解答](https://sy8pzmhmun.feishu.cn/wiki/SLGwwqgzIiEuC3kwmV8cSZY0nTg?from=from_copylink)  
...
<details>
  <summary>Kea2的性质是什么含义？Kea2意义和价值是什么？</summary>
    ...
    Kea2另一个重大的价值是，它解决了上述思想所需要的技术支撑，比appium更轻量的UI编写方式，fastbot编写性质的能力不足，以及无法编写逻辑和断言。整体上是保留了fastbot以往的优秀品质，完善了其不足和短板。
...
请联系Xixian Liang，邮箱：[xixian@stu.ecnu.edu.cn](xixian@stu.ecnu.edu.cn)，并附上你的微信号或二维码，以便邀请加入微信群。
