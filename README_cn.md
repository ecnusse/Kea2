...
Kea2（及其理念）已被以下项目使用或集成：

- [OPay Business](https://play.google.com/store/apps/details?id=team.opay.pay.merchant.service) — 一款金融及支付应用（每日活跃用户达200万）。OPay使用Kea2在POS机和移动设备上进行回归测试。
...
如果之前已安装Kea2，可通过以下命令升级至最新版本：

```bash
python3 -m pip install -U kea2-python
```

> 如果你使用清华、科大等镜像站点，可能会升级失败，因为这些站点可能尚未同步最新版本。此时可以尝试手动指定最新版本安装，或直接使用 `pip install kea2-python -i https://pypi.org/simple`。
...
> 如果是首次运行Kea2，此步骤必需。升级后建议重新执行此步骤以确保新配置生效。
...
Kea2支持复用已有的UI测试脚本。我们的灵感来自于：*已有的UI测试脚本通常覆盖重要的App功能且能到达深层App状态，因此它们可以作为良好的“引导脚本”，驱动Fastbot探索重要且深层的App状态。*

例如，你可能已有“登录并添加好友”的UI测试脚本。此特性允许你使用已有脚本，在脚本中设置断点（即可中断点），并在每个断点后启动Fastbot探索App。这样你可以先完成登录，再启动Fastbot探索登录后的App状态，帮助Fastbot探索深层App状态（Fastbot自身难以轻松完成登录操作）。
...
3. 你需要将以下代码模板插入到已有测试用例中：你可以在注释部分添加自己的hook逻辑，包括启动或停止appium会话、清理实例等，具体取决于你的setup和teardown设计。除此之外，只需配置`option`参数和`configs_path`参数（即`configs`目录所在路径，通常是执行`kea2 init`的目录），然后传入`run_kea2_testing`函数即可。
...
if os.environ.get('KEA2_HYBRID_MODE', '').lower() == 'true': 
    ...
    return  # 这会使该测试用例后续步骤不执行
```

## 测试报告

Kea2会在每次测试结束后自动生成HTML测试报告。你可以在工作目录下的`output/`目录中找到报告。

你也可以通过命令 `kea2 report` 手动生成测试报告（详情见 `kea2 report -h`）。  
使用 `-s/--sync` 选项可在生成报告前从设备同步数据，使用 `-p/--path` 指定一个或多个结果目录。  
例如：  
`kea2 report -p output/res_YYYYMMDDHH_xxxxxx`  
`kea2 report -s -p output/res_YYYYMMDDHH_xxxxxx`

你还可以通过命令 `kea2 merge` 合并多个测试会话的报告（详情见 `kea2 merge -h`）。  
合并报告在多次测试时非常有用。

你可以查看Opay提供的[测试报告示例](https://ecnusse.github.io/Kea2_sample_report/)(感谢！)。  
